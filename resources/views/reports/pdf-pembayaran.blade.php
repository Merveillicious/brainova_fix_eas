...
<head>
<meta charset="UTF-8">
<title>Laporan Pembayaran - Brainova</title>
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .meta { display: flex; justify-content: space-between; padding: 0 24px; margin-bottom: 16px; font-size: 11px; color: #555; }
...
<body>
<div class="header">
    <h1>📋 Laporan Pembayaran — Brainova</h1>
...
<div class="summary">
    ...
    <div class="summary-card">
        <div class="val">Rp {{ number_format($totalBerhasil, 0, ',', '.') }}</div>
        <div class="lbl">Total Pendapatan Berhasil</div>
    </div>
    <div class="summary-card">
        <div class="val">{{ $totalMenunggu }}</div>
        <div class="lbl">Menunggu Konfirmasi</div>
    </div>
    <div class="summary-card">
        <div class="val">{{ $bookings->where('status_booking', 'selesai')->count() }}</div>
        <div class="lbl">Booking Selesai</div>
    </div>
</div>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Rp {{ number_format($b->payment->jumlah ?? 0, 0, ',', '.') }}</td>
            <td>
                <span class="badge badge-{{ $b->payment->status ?? 'menunggu' }}">
                    {{ ucfirst($b->payment->status ?? 'menunggu') }}
                </span>
            </td>
